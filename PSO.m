...
if fym < max(fxm)
    ...
    vInitial = vInitial * weightFactor + selfFactor * rand * (xm - x) + crowdFactor * rand * (repmat(ym, InitialNum, 1) - x);% 速度更新
                                        % 边界速度处理
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure(3);plot(record);title('收敛过程记录')
x0 = xLower : Interpolation : xTop;
figure(4);plot(x0, f(x0), 'k-', x, f(x), 'ro');title('最终状态位置')
